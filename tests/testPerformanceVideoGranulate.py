...
class VideoGranulateTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.video_key = response.video_key
        self.grains_key = response.grains_key
        self.uid_list.append(self.grains_key)
        self.uid_list.append(self.video_key)

    @time_it('Granulation of a video.')
    def testGranulate(self):
        while not self.video_granulate_service.get(key=self.grains_key).resource().done:
            sleep(1)
